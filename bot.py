...
from helpers.database import is_authorized_chat

botStartTime = time.time()

async def check_authorization(m: Message) -> tuple[bool, str]:
    """
    ...
    - Normal users: Only allowed in authorized groups
    """
    user_id = m.from_user.id
    chat_id = m.chat.id
    
    # Owner always authorized everywhere
    if user_id == int(Config.OWNER):
        return True, ""
    
    # Private chat check
    if chat_id == user_id:
        # Normal user trying to use in private chat
        return False, (
    ...
    # Group chat - check if authorized
    is_auth = await is_authorized_chat(chat_id)
    if not is_auth:
        return False, (
            "🚫 **Unauthorized Group**\n\n"
            "❌ This group is not authorized to use this bot.\n\n"
            f"📞 Contact owner: @{Config.OWNER_USERNAME} to authorize this group."
        )
...
@mergeApp.on_message(filters.command(["start"]))
async def start_handler(c: Client, m: Message):
    """Fixed start handler with group-only authorization"""
    user = UserSettings(m.from_user.id, m.from_user.first_name)
    
    LOGGER.info(f"Start command - User: {user.user_id}, Chat: {m.chat.id}, Allowed: {user.allowed}")
    
    # Check authorization (group-only for normal users)
    is_authorized, error_msg = await check_authorization(m)
    if not is_authorized:
        await m.reply_text(error_msg, quote=True)
        return
...
async def video_upload_handler(c: Client, m: Message):
    """Handle video uploads with download mode enforcement and group authorization"""
    user = UserSettings(m.from_user.id, m.from_user.first_name)
