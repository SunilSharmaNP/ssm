...
@mergeApp.on_message(filters.text & filters.private & ~filters.command(['start', 'help', 'login', 'encode', 'settings']))
async def url_handler(c: Client, m: Message):
    """Handle URL downloads and add to queue"""
    user = UserSettings(m.from_user.id, m.from_user.first_name)
    ...
    if not (text.startswith('http://') or text.startswith('https://')):
        await m.reply_text(
            "ℹ️ **Not a valid URL**\n\n"
            "💡 Send a direct download link to add video to queue.\n"
    ...
    try:
        ...
        LOGGER.info(f"📥 URL Download started: {text}")
        downloaded_file = await download_from_url(
            url=text,
        ...
        if not downloaded_file:
            await status_msg.edit_text(
                "❌ **Download Failed!**\n\n"
                "🚨 Could not download from the provided URL.\n"
                "💡 Make sure it's a valid direct download link."
            )
            return
        
        LOGGER.info(f"✅ URL Download completed: {downloaded_file}")
        
        # Create a pseudo-message object to store URL download info
        # We'll store the message ID in queue and the downloaded file path in a global dict
        import __init__
        if not hasattr(__init__, 'downloaded_urls'):
            __init__.downloaded_urls = {}
        
        # Store the downloaded file path with message ID as key
        __init__.downloaded_urls[m.id] = downloaded_file
        
        # Add to queue (using message ID as reference)
        ...
        await status_msg.edit_text(
            f"✅ **Video Downloaded & Added to Queue!**\n\n"
            f"📁 **File:** `{file_name}`\n"
            ...
            f"{'🚀 Ready to merge/encode!' if video_count >= 1 else '📥 Add more videos'}",
            reply_markup=keyboard
        )
