...
async def uploadVideo(c, cb, merged_video_path: str, width: int, height: int, 
                     duration: int, video_thumbnail: str, file_size: int, upload_mode: bool):
    """
    Backward compatibility wrapper for uploadVideo function.
    This wraps the new upload_to_telegram function with the old signature.
    
    Args:
        c: Pyrogram Client
        cb: CallbackQuery
        merged_video_path: Path to the merged video file
        width: Video width
        height: Video height
        duration: Video duration in seconds
        video_thumbnail: Path to thumbnail image
        file_size: File size in bytes
        upload_mode: True for document mode, False for video mode
    """
    # Extract filename from path (without extension)
    filename_base = os.path.splitext(os.path.basename(merged_video_path))[0]
    
    # Call the new upload_to_telegram function
...
async def uploadFiles(c, cb, up_path: str, n: int = 1, all: int = 1):
    """
    Upload files (for streams extraction and other multi-file uploads).
    
    Args:
        c: Pyrogram Client
        cb: CallbackQuery
        up_path: Path to file to upload
    ...
    try:
        ...
        print(f"Upload failed for {up_path}: {e}")
        await cb.message.edit_text(
            f"❌ **Upload Failed**\n\n"
            f"📁 **File:** `{file_name}`\n"
            f"🚨 **Error:** `{str(e)}`"
