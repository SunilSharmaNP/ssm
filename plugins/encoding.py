...
async def start_encoding_process(c: Client, cb: CallbackQuery, user_id: int):
    ...
    try:
        ...
        try:
            ...
            # Check if this is a URL-downloaded video
            import __init__
            if hasattr(__init__, 'downloaded_urls') and video_msg_id in __init__.downloaded_urls:
                # Video was downloaded from URL, use the existing file
                input_file = __init__.downloaded_urls[video_msg_id]
                LOGGER.info(f"Using URL-downloaded file: {input_file}")
            else:
                # Regular Telegram video/document, download it
                # Get original filename
                if video_msg.video:
                    original_name = video_msg.video.file_name or f"video_{video_msg_id}.mp4"
                elif video_msg.document:
                    original_name = video_msg.document.file_name or f"video_{video_msg_id}.mp4"
                else:
                    original_name = f"video_{video_msg_id}.mp4"
                
                # Download to user directory with proper filename
                input_file = await video_msg.download(file_name=f"{user_dir}/{original_name}")
        ...
        # Prepare output file
        file_ext = os.path.splitext(input_file)[1] or ".mp4"
